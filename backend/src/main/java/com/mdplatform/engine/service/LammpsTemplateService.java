...
public class LammpsTemplateService {
    ...
    private Map<String, Object> buildTemplateContext(Long userId, Long jobId,
        ...
        // ========== 各阶段步数 ==========
        // 生产配置：足够的模拟时间确保物性计算收敛
        // nsteps_nvt: NVT平衡步数，50ps用于温度平衡
        // nsteps_npt: NPT平衡步数，50ps用于压力和密度平衡
        // nsteps: 生产模拟步数，1ns轨迹用于Green-Kubo电导率计算
        context.put("nsteps_nvt", 50000);
        context.put("nsteps_npt", 50000);
        context.put("nsteps", 1000000);
        ...
        context.put("nsteps_nve_small", 2000);
        context.put("nsteps_nve_medium", 2000);
        context.put("nsteps_nve_full", 2000);
        ...
        if (input != null && input.getOutputFrequencyStep() != null) {
            ...
            // dump_freq：生产模拟需要足够的轨迹帧用于Green-Kubo自相关计算
            // 1000000步/1000步=1000帧，足够电导率自相关函数充分衰减
            // 同时避免轨迹文件过大（770原子*1000帧 ≈ 60MB）
            context.put("dump_freq", Math.min(outputFreq, 1000));
            context.put("ave_freq", outputFreq);
        } else {
            context.put("thermo_freq", 1000);
            context.put("dump_freq", 1000);
            context.put("ave_freq", 1000);
        }
