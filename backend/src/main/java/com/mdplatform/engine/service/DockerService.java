...
public class DockerService {
    ...
    public String runLAMMPS(String inputFilename, String jobId, String containerWorkDir,
        ...
        try {
            ...
            // 阶段4：构建LAMMPS命令
            // GPU加速策略：
            //   - 生产阶段（in.production）：使用lj/cut/coul/cut力场，GPU包兼容，启用GPU加速
            //   - 最小化阶段（in.minimization）：使用soft pair_style，GPU不支持，使用CPU模式
            //   - 平衡阶段（in.equilibrium）：包含NVE/limit等GPU不支持的fix，使用CPU模式
            //   - GPU模式参数：-sf gpu -pk gpu 1，自动将支持的pair style替换为GPU版本
            //   - 当GPU不可用时，自动回退到CPU模式
            boolean useGpu = gpuDiagnostic && inputFilename.contains("production");
            log.info("[LAMMPS执行] 阶段4：构建LAMMPS命令，GPU加速: {}（输入文件: {}）", useGpu, inputFilename);
            ...
            List<String> lammpsCmd;
            if (useGpu) {
                // 生产阶段使用GPU加速：-sf gpu自动替换pair style为GPU版本，-pk gpu 1指定1个GPU
                lammpsCmd = Arrays.asList(
                        "lmp",
                        "-sf", "gpu",
                        "-pk", "gpu", "1",
                ...
                log.info("[LAMMPS执行] 使用GPU加速模式执行LAMMPS（生产阶段）");
            } else {
                // 最小化/平衡阶段使用CPU模式（soft pair_style和NVE/limit不支持GPU）
                lammpsCmd = Arrays.asList(
                        "lmp",
                        "-in", containerInputPath,
                        "-log", logFilePath,
                ...
                log.info("[LAMMPS执行] 使用CPU模式执行LAMMPS（非生产阶段）");
            }
