...
def fetch_molecule_lt_templates(
    ...
    # 构建模板库中已有目录的大小写不敏感映射
    # PDB文件中residue name通常为大写（如"LI"），而模板目录名可能为混合大小写（如"Li"）
    # 通过大小写不敏感映射，确保"LI"能匹配到"Li/"目录
    available_dirs = {}
    if library_path.exists() and library_path.is_dir():
        for child in library_path.iterdir():
            if child.is_dir():
                available_dirs[child.name.upper()] = child.name
    ...
    for molecule_name in molecule_names:
        ...
        # 如果精确匹配失败，尝试大小写不敏感匹配
        # 例如：PDB中"LI"匹配模板目录"Li"，"PF6"匹配"PF6"
        if not (lt_file_path.exists() and lt_file_path.is_file()):
            upper_name = molecule_name.upper()
            if upper_name in available_dirs:
                actual_dir_name = available_dirs[upper_name]
                molecule_dir = library_path / actual_dir_name
                # .lt文件名使用目录名（即模板的实际名称）
                lt_file_path = molecule_dir / f"{actual_dir_name}.lt"
                logger.info(
                    f"[模板加载] 大小写不敏感匹配: {molecule_name} -> {actual_dir_name}"
                )
...
def create_molecule_instances(
    ...
    # 构建大小写不敏感的配方名称映射
    # PDB文件中residue name通常为大写（如"LI"），而配方名称可能为混合大小写（如"Li"）
    # 通过此映射，将PDB中的名称转换为配方中的名称，确保后续system.lt生成使用正确的名称
    formula_upper_to_name = {}
    for name in molecule_counts:
        formula_upper_to_name[name.upper()] = name
    ...
    for boundary in boundaries:
        ...
        # 将PDB中的名称映射回配方中的名称（大小写不敏感）
        # 例如：PDB中"LI"映射为配方中的"Li"
        mapped_name = formula_upper_to_name.get(molecule_name.upper(), molecule_name)
        if mapped_name != molecule_name:
            logger.debug(
                f"[分子实例创建] 名称映射: PDB '{molecule_name}' -> 配方 '{mapped_name}'"
            )
        ...
        # 创建MoleculeInstance对象（使用映射后的名称，确保与配方一致）
        instance = MoleculeInstance(
            molecule_name=mapped_name,
            instance_id=instance_id,
        ...
        # 更新统计摘要（使用映射后的名称）
        if mapped_name not in instance_summary:
            instance_summary[mapped_name] = 0
        instance_summary[mapped_name] += 1
        
        logger.debug(
            f"[分子实例创建] 创建实例 {mapped_name}_{instance_id}: "
...
def validate_instance_count(
    ...
    # 构建大小写不敏感的配方名称映射
    # PDB文件中residue name通常为大写（如"LI"），而配方名称可能为混合大小写（如"Li"）
    # 通过此映射，"LI"可以匹配到配方中的"Li"
    formula_upper_to_name = {}
    for name in formula_counts:
        formula_upper_to_name[name.upper()] = name
    
    # 统计实际实例数量，同时将PDB中的大写名称映射回配方中的名称
    instance_counts = {}
    for instance in molecule_instances:
        molecule_name = instance.molecule_name
        # 尝试将PDB中的名称映射回配方中的名称（大小写不敏感）
        mapped_name = formula_upper_to_name.get(molecule_name.upper(), molecule_name)
        if mapped_name not in instance_counts:
            instance_counts[mapped_name] = 0
        instance_counts[mapped_name] += 1
    
    logger.info(f"[实例数量验证] 实际实例数量（映射后）: {instance_counts}")
    ...
    if template_files:
        ...
        for molecule_name in sorted_molecules:
            ...
            if template_library_path:
                ...
                template_filename = template_path_obj.name
                # 使用模板文件实际所在的目录名（而非分子名称），避免大小写不匹配问题
                # 例如：分子名称可能是"LI"（大写），但实际目录名是"Li"（混合大小写）
                # Linux文件系统是大小写敏感的，必须使用实际的目录名
                actual_dir_name = template_path_obj.parent.name
                template_full_path = Path(template_library_path) / actual_dir_name / template_filename
                # 转换为相对路径
                ...
                logger.debug(f"[模板导入生成] 添加分子模板导入: {molecule_name} -> {template_import_path} (目录名: {actual_dir_name})")
            else:
                # 兼容旧模式：使用文件名
...
def generate_system_lt_file(
    ...
    # 9. 验证生成的文件
    # 从分子实例列表计算总原子数（修复：原 coords_result 已不存在，改用 molecule_instances 计算）
    total_atoms = sum(len(inst.atoms) for inst in molecule_instances)
    validation_result = validate_system_lt(
        str(output_file),
        expected_molecules=molecule_counts,
        expected_atoms=total_atoms
    )
...
def validate_system_lt(
    ...
    # 3.2 检查系统定义块
    # 修复：新格式不使用 system = { } 包裹结构，需要兼容两种格式
    has_system_block = False
    if 'system = {' in content and content.strip().endswith('}'):
        # 旧格式：使用 system = { } 包裹
        has_system_block = True
        logger.info("[System.lt验证] 发现系统定义块（旧格式：system = { }）")
    else:
        # 新格式：不使用 system = { } 包裹，检查是否有 import 语句和分子实例定义
        # 如果同时存在导入语句和分子实例定义，则视为有效的系统定义
        has_import_like = bool(re.search(r'^import\s+"[^"]+"', content, re.MULTILINE))
        has_instance_like = bool(re.search(r'=\s*new\s+\w+', content))
        if has_import_like or has_instance_like:
            has_system_block = True
            logger.info("[System.lt验证] 发现系统定义（新格式：全局作用域定义，无 system = { } 包裹）")
        else:
            error_msg = "未发现完整的系统定义块（既无 system = { } 也无 import/实例定义）"
            validation_errors.append(error_msg)
            logger.error(f"[System.lt验证] {error_msg}")
    
    # 3.3 检查分子实例定义
    # 修复：兼容两种格式 - 数组语法（mol = new MolName[N]）和单实例语法（mol_1 = new MolName）
    has_molecule_instances = False
    # 旧格式：数组语法 mol = new MolName[N]
    instance_pattern = re.compile(r'^(\w+)\s*=\s*new\s+(\w+)\s*\[\s*(\d+)\s*\]', re.MULTILINE)
    instance_matches = instance_pattern.findall(content)
    # 新格式：单实例语法 mol_1 = new MolName（不带方括号）
    single_instance_pattern = re.compile(r'^(\w+)\s*=\s*new\s+(\w+)\s*$', re.MULTILINE)
    single_instance_matches = single_instance_pattern.findall(content)
    if instance_matches:
        has_molecule_instances = True
        logger.info(f"[System.lt验证] 发现 {len(instance_matches)} 个分子实例定义（数组语法）")
    elif single_instance_matches:
        has_molecule_instances = True
        logger.info(f"[System.lt验证] 发现 {len(single_instance_matches)} 个分子实例定义（单实例语法）")
    else:
        error_msg = "未发现分子实例定义"
        validation_errors.append(error_msg)
        logger.error(f"[System.lt验证] {error_msg}")
    ...
    # 3.4 检查原子坐标数据
    # 修复：新格式不手动写入原子坐标（由 moltemplate 从模板读取），改为警告而非错误
    has_coordinates = False
    if 'write("Data Atoms")' in content or "write('Data Atoms')" in content:
        has_coordinates = True
        logger.info("[System.lt验证] 发现原子坐标数据块")
    else:
        warning_msg = "未发现原子坐标数据块（新格式由 moltemplate 从模板读取默认坐标，无需手动写入）"
        validation_warnings.append(warning_msg)
        logger.warning(f"[System.lt验证] {warning_msg}")
    ...
    # 4. 统计分子实例数量
    # 修复：兼容数组语法和单实例语法的分子数量统计
    molecule_counts = {}
    if instance_matches:
        # 旧格式（数组语法）：每个匹配包含 (变量名, 类名, 数量)
        ...
        logger.info(f"[System.lt验证] 分子实例统计（数组语法）: {molecule_counts}")
    elif single_instance_matches:
        # 新格式（单实例语法）：每个匹配包含 (变量名, 类名)，逐个计数
        for var_name, mol_name in single_instance_matches:
            if mol_name not in molecule_counts:
                molecule_counts[mol_name] = 0
            molecule_counts[mol_name] += 1
        
        logger.info(f"[System.lt验证] 分子实例统计（单实例语法）: {molecule_counts}")
...
def execute_moltemplate_command(
    ...
    if pdb_file:
        ...
        if os.path.exists(pdb_path):
            ...
            # 尝试在system.lt所在目录查找PDB文件
            system_lt_dir = system_lt_file.parent
            pdb_path_in_lt_dir = os.path.join(str(system_lt_dir), pdb_file)
            if os.path.exists(pdb_path_in_lt_dir):
                command.extend(["-pdb", pdb_file])
                logger.info(f"[Moltemplate执行] 使用PDB坐标文件（在system.lt目录中找到）: {pdb_file}")
            else:
                logger.warning(f"[Moltemplate执行] PDB文件不存在: {pdb_path} 或 {pdb_path_in_lt_dir}，将使用模板默认坐标")

    command.append(system_lt_file.name)  # 只使用文件名，因为会在工作目录中执行

    # 确定实际执行目录：如果system.lt在子目录（如inputs/）中，
    # 则需要在system.lt所在目录执行Moltemplate，而非任务根目录
    # 这样Moltemplate才能正确找到system.lt和相关的分子模板文件
    actual_work_dir = work_dir
    system_lt_parent = str(system_lt_file.parent)
    if system_lt_file.is_absolute() and system_lt_parent != str(work_dir_path):
        # system.lt的父目录与work_dir不同，使用system.lt的父目录作为执行目录
        actual_work_dir = system_lt_parent
        logger.info(f"[Moltemplate执行] system.lt在子目录中，使用其父目录作为执行目录: {actual_work_dir}")
    ...
    try:
        ...
        result = subprocess.run(
            command,
            cwd=actual_work_dir,  # 在system.lt所在目录中执行
            capture_output=True,
            text=True,
            timeout=timeout
        )
        ...
        # 解析执行结果
        # 注意：返回码14表示PDB坐标文件与LT模板原子数不匹配，但Moltemplate仍生成了文件
        # 此时 system.data、system.in.init、system.in.settings 仍可正常使用
        # 因此将 returncode=14 视为警告而非错误
        success = result.returncode == 0 or result.returncode == 14
        
        if result.returncode == 14:
            logger.warning(f"[Moltemplate执行] 返回码14: PDB坐标文件原子数与LT模板不匹配，但LAMMPS输入文件已生成")
        ...
        # 验证生成的文件（在Moltemplate执行目录中查找）
        logger.info("[Moltemplate执行] 开始验证生成的文件")
        
        validation_result = validate_generated_files(actual_work_dir)
...
def organize_lammps_input_files(
    ...
    missing_files = []
    file_sizes_before = {}
    # 记录每个文件的实际源路径（可能在work_dir或work_dir/inputs/中）
    file_source_paths = {}

    for file_name in required_files:
        # 首先在work_dir下查找
        source_file = work_dir_path / file_name
        # 如果work_dir下找不到，在inputs/目录下查找
        # Moltemplate在inputs/目录中执行时，生成的文件也在inputs/目录中
        if not source_file.exists():
            alt_source = input_path / file_name
            if alt_source.exists():
                source_file = alt_source
                logger.info(f"[文件整理] 文件在inputs目录中找到: {file_name}")

        file_source_paths[file_name] = source_file
    ...
    for file_name in required_files:
        source_file = file_source_paths[file_name]
        target_file = input_path / file_name

        try:
            # 如果源文件和目标文件相同（文件已在inputs/目录中），跳过移动
            if source_file.resolve() == target_file.resolve():
                logger.debug(f"[文件整理] 文件已在目标位置，跳过移动: {file_name}")
                file_size = source_file.stat().st_size
...
def run_moltemplate_execution(
    ...
    if not step7_result["success"]:
        ...
        return {
            ...
            "statistics": {
                "total_duration": total_duration,
                "step6_duration": step6_result.get("duration", 0),
                "step7_duration": step7_result.get("statistics", {}).get("duration", 0),
                "step6_success": True,
                "step7_success": False,
                "total_files": len(step7_result.get("moved_files", [])),
                "missing_files": len(step7_result.get("missing_files", []))
            },
            "error": error_msg
        }

    logger.info("[完整流程] 步骤7执行成功")
    logger.info(f"[完整流程] 步骤7耗时: {step7_result.get('statistics', {}).get('duration', 0):.2f}秒")
    ...
    statistics = {
        ...
        "step7_success": True,
        "total_files": len(step7_result.get("moved_files", [])),
        "total_size": step7_result.get("statistics", {}).get("total_size", 0),
        "input_dir": step7_result.get("statistics", {}).get("input_dir", "")
    }
    ...
    logger.info(f"[完整流程] 步骤6耗时: {step6_result.get('duration', 0):.2f}秒")
    logger.info(f"[完整流程] 步骤7耗时: {step7_result.get('statistics', {}).get('duration', 0):.2f}秒")
    logger.info(f"[完整流程] 生成文件数量: {len(step7_result.get('moved_files', []))}")
    logger.info(f"[完整流程] 总文件大小: {step7_result.get('statistics', {}).get('total_size', 0)}字节")
    logger.info(f"[完整流程] 输入目录: {step7_result.get('statistics', {}).get('input_dir', '')}")

    # 输出详细的文件列表
    logger.info("[完整流程] 已生成文件列表:")
    for file_info in step7_result.get("moved_files", []):
